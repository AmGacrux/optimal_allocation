...
namespace allocation_optimize_NS {
	...
	public:
		...
		public:
			...
			//std::vector<std::map<int, DC*>> adjacentNodes;
		...
		public:
			...
			std::vector<std::vector<Problem::DC *>> routes; // このリストのsizeはTask tiのsizeと一致する(筈)
			/*
			...
			void printResultCost() {
				...
				std::cout << "Total communication cost = " << resultCommCost << std::endl;
			...
			void calcTotalCost(Problem *p) {
				resultCompCost = [=] {
					int tmp{ 0 };
					for (auto assignedPair : allocationList)
						tmp += assignedPair.first->cmp_r*assignedPair.second->cmp_c;
					return tmp;
				}();
				...
				// 配置したタスクごとにパスを辿ってCmmCostを計算する
			}
		}; // Def: class Solution end 
	...
	*/
	...
	std::vector<Problem::DC *> dfsSearch(Problem *p, Problem::DC *src, Problem::DC *dst) {
		std::vector<std::list<Problem::DC *>> allPaths{};
		std::vector<Problem::DC *> path{};
	...
	Problem::Solution* greedy(Problem *p) {
		...
		for (uint32_t i = 0; i < solution->allocationList.size(); ++i) {
			if (i == solution->allocationList.size() - 1) break;
			std::vector<Problem::DC *> path = dfsSearch(p, solution->allocationList[i].second, solution->allocationList[i + 1].second);
			solution->routes.push_back(path);
		}
		std::cout << "path amount: " << solution->routes.size() << std::endl;

		//routes = dfsSearch(p, p->dj[0], p->dj[p->dj.size()-1]);
		//	path = dfsSearch(p, p->dj[p->dj.size()-1], p->dj[0]);
		for (auto path : solution->routes) {
			for (auto e : path) {
				std::cout << e->idx << "->";
			}
			std::cout << std::endl;
